...
void write_floor_loop(BITMAP * src, int x0, int y0)
{
   ...
   long x, y, n=0, start_header, start_data, end_data;
   int  i, present[25];
...
void read_ini_line(LINE_TYPE lt, void * ptr)
{
   ...
   if ( (lt >= LT_F1) && (lt <= LT_F5) )
   {
      if (sscanf(tmpline + c, "%02X %02X %02X %02X %02X",
         ptr, ((char *) ptr)+1, ((char *) ptr)+2, ((char *) ptr)+3, ((char *) ptr)+4) != 5)
      {
...
void read_ini_all(void)
{
   ...
   for (b=0; b<block_number; b++)
   {
      ...
      read_ini_line(LT_YD, &block.roof_y);
...
int main(int argc, char * argv[])
{
   ...
   int  i, drive_pos = -1, last_slash_pos = -1, period_pos = -1, n;
   
   // header & syntaxe & atexit()
   printf("DT1MAKE v 0.97b beta, by Paul Siramy, Freeware\n"
          "==============================================\n");
