...
char gamma_str[GC_MAX][5] = // gamma correction string table
        {
...
void read_gamma(void)
{
   FILE * in;
   int  gt, i, v;
   char gamma_path[] = "./res/pal/gamma.dat";
...
void load_palette(char * name)
{
   ...
   d2_pal[0][0] = d2_pal[0][1] = d2_pal[0][2] = 60;
...
int main(int argc, char * argv[])
{
   ...
   // header & syntaxe
   printf("DT1DEBUG v 0.97c, by Paul Siramy, Freeware\n"
          "==========================================\n");
   if (argc == 1) {
   ...
   if (strcmp(argv[2], "-p") == 0) {
      ...
      load_palette("./res/d2pal/act1.dat");
      pal_name = d2palname[0];
   }

   // use the current gamma correction value to make a new palette
   pal_d2_2_vga();
   ...
   while ( ! done) {
      ...
      // grid on top of tile
      if (layer == 1) {
         draw_block(tmpbmp, plotx, ploty, b);
      }
      ...
      sprintf(tmp, "roof Y   : %04lX ( = %i)", block_ptr->roof_y, block_ptr->roof_y);
