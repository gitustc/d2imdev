...
void d2p_chunk_info(const char *filename, uint32_t *filesize, )
...
uint32_t d2p_file_index(D2PKG *pkg, const char *filename)
{

    D2PCHKINFO  *chunkInfo;


    uint32_t    nameHash = _string_to_hash(filename);
    uint32_t    hashIndex = (nameHash & pkg->hash_mask);

	uint32_t    fileIndex = (pkg->hash_table).table[hashIndex];

	while (fileIndex >= 0)
	{
        
        chunkInfo = _get_chunk_info(pkg, fileIndex);
		if(chunkInfo.name_hash == nameHash){
            if(chunkInfo.other_info != 0){
                return -1;
            }else{
                return fileIndex;
            }
        }else{
            if(++hashIndex >= (pkg->hash_table).size){
                hashIndex = 0;
            }
            fileIndex = (pkg->hash_table).table[hashIndex];
		}
	}
	return -1;
}
...
static uint32_t _string_to_hash(const char *str, uint32_t seed)
{
    uint32_t out = 0;
...
static uint32_t _write_to_end(D2PKG *pkg, D2PBLKINFO *bip)
{

    void *dataChunk[MAX_CHUNK_SIZE];
    uint32_t chunkCount = 

    fseek(pkg->fp, SEEK_END);
