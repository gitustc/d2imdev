...
inline int keyval_to_scancode(int keyval){
    return keyval >> 8;
}
inline char kyeval_to_ctrl_ascii(int keyval){
    return (keyval & 0xff) + 'a' - 1;
}
...
void _interfac_user_handler(int start_ds1_idx)
{
   int  ds1_idx, done, cx, cy, n, i, dx, dy, old_ds1_idx=0;
   int  old_mouse_x = mouse_x, old_mouse_y=mouse_y, old_mouse_b=0;
   int  cur_mouse_z = 0, old_mouse_z = 0;
