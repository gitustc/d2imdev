...
int anim_load_dcc( char  * name, COF_S * cof, int   lay_idx, long  user_dir, UBYTE * palshift )
{
    ...
    //  这个函数就做了这么一件事情:
    //      1.分配dcc结构体内存
    //      2.把buff中的len长字节拷贝到dcc->ptr : memcpy(dcc->ptr, mem_ptr, mem_size);
    //      3.返回dcc指向内存, 需要手动释放
    //  }}}
    //  PREPARE DCC
...
COF_S * anim_load_cof(char * base, char * tok, char * mod, char * clas, long user_dir, int obj_line, int progress)
{
    ...
    //艹..这么奇葩的缩写
    //每个朝向的frame数量
    //我勒个擦擦..我居然看到了一个cof->fpd == 60 的....
    cof->fpd = * bptr;
    //从这里看的话...貌似是一个动作就有60...
    //不然不能这样rand()一下的...除非是一类动作..比如站着和站着摇头之类
    ...
    for (i=0; i < cof->lay; i++) {
        // composit index
        // 很奇怪, 这个lay是神马
        // 上面也看到了, lay这个变量1,2,3不等
        // 每个分量都有shadow, transparency, 还有weapon这三个分量
        //
        // size = cof->dir * cof->fpd * cof->lay;
        // cof->priority = (UBYTE *) malloc(size);
        //
        // 再从这里来看..每一frame都有lay个priority
        // 我现在还是不太明了每一个lay到底是神马
        // 应该是同一种东西的不同形态???
        // 根据后面看，应该是一个objects是多个lay（分片）重叠而成的
        idx = * bptr;
        bptr++;
...
void anim_update_gfx(int progress)
{
    ...
    for (o=0; o < glb_ds1.obj_num; o++) {
        idx = glb_ds1.obj[o].desc_idx;
        if (idx != -1) {
            // 统计obj.txt内的obj的使用次数，按照desc_idx来统计
    ...
    for (od=0; od < txt->line_num; od++) {
        ...
        if (glb_ds1edit.obj_desc[od].usage_count == 0) {
            ...
            if (cof == NULL) {
                //每个使用中的obj都需要有一个cof
                //没有的话就去load一个 这里面也load了实际的人物或者怪物的图像
                // no cof, so load it
    ...
    for (o=0; o < glb_ds1.obj_num; o++) {
        ...
        if (idx != -1) {
            if (glb_ds1edit.obj_desc[idx].cof != NULL) {
                ds1_anim_used = TRUE;
                o = glb_ds1.obj_num; // stop the check of all objects
            }
        }
    }
}
